...
export default function ProjectPage() {
  ...
  const [votes, setVotes] = useState<ProjectVotes | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const [projectResult, paymentsResult] = await Promise.all([
        getHackProjectById(params.id as string),
        getProjectSplitPayments(params.id as string)
      ]);

      if (projectResult.success && projectResult.project) {
        setProject(projectResult.project);
        await fetchProjectVotes(projectResult.project);
        await fetchRepoData(projectResult.project.repository);
      }
  ...
  const fetchRepoData = async (repoUrl: string | null) => {
    if (!repoUrl) return;
  ...
  const handleVote = async (voteType: "contributor" | "maintainer") => {
    ...
    try {
      ...
      if (result.success) {
        ...
        await fetchProjectVotes(project);
      } else {
  ...
  const calculateSplit = () => {
    if (!votes || votes.totalVotes === 0) {
      return { contributorShare: 50, maintainerShare: 50 };
    }

    const contributorPercentage = (votes.contributorVotes / votes.totalVotes) * 100;
    const maintainerPercentage = (votes.maintainerVotes / votes.totalVotes) * 100;
  ...
  const getVotePercentage = (type: "contributor" | "maintainer") => {
    ...
    const voteCount = type === "contributor" ? votes.contributorVotes : votes.maintainerVotes;
    return (voteCount / votes.totalVotes) * 100;
  };
                        ...
                        <Card>
                          ...
                          <CardContent>
                            <div className="space-y-4">
                              <div className="text-center">
                                <div className="text-3xl font-bold text-purple-600">{votes.totalVotes}</div>
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span>{votes.contributorVotes} ({getVotePercentage("contributor").toFixed(1)}%)</span>
                                </div>
                                <Progress value={getVotePercentage("contributor")} className="h-2" />
                                
                                <div className="flex justify-between text-sm">
                                  <span>Maintainers</span>
                                  <span>{votes.maintainerVotes} ({getVotePercentage("maintainer").toFixed(1)}%)</span>
                                </div>
                      ...
                      <Card>
                        ...
                        <CardContent>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="text-center">
                              <div className="text-2xl font-bold text-blue-600">{votes.contributorVotes}</div>
                            ...
                            <div className="text-center">
                              <div className="text-2xl font-bold text-green-600">{votes.maintainerVotes}</div>
                            ...
                            <div className="text-center">
                              <div className="text-2xl font-bold text-purple-600">{votes.totalVotes}</div>
                              <div className="text-sm text-neutral-600 dark:text-neutral-400">Total Votes</div>
                              {votes.userVote && (
                                <Badge variant="default" className="mt-1 bg-green-100 text-green-800">
                                  <CheckCircle className="w-3 h-3 mr-1" />
                                  You voted: {votes.userVote}
                                </Badge>
                              )}
                            </div>
                          </div>
                        ...
                        <CardContent>
                          <div className="flex gap-4">
                            <Button
                              ...
                              variant={votes.userVote === "contributor" ? "default" : "outline"}
                              className="flex-1"
                            >
                              {votingLoading ? (
                                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                              ) : votes.userVote === "contributor" ? (
                            ...
                            <Button
                              ...
                              disabled={!session?.user?.username || votingLoading}
                              variant={votes.userVote === "maintainer" ? "default" : "outline"}
                              className="flex-1"
                            >
                              {votingLoading ? (
                                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                              ) : votes.userVote === "maintainer" ? (
                                <CheckCircle className="w-4 h-4 mr-2" />
                              ) : (
                                <Circle className="w-4 h-4 mr-2" />
                              )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {votes && votes.totalVotes > 0 && splitAmount && (
                        ...
                        <Button
                          onClick={handleSplit}
                          disabled={!splitAmount || !address || !votes || votes.totalVotes === 0 || splittingLoading}
                          className="w-full"
                        >
                          {splittingLoading ? (
                            <Loader2 className="w-4 h-4 mr-2 animate-spin" />
