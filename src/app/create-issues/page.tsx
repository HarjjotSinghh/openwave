...
const createContract = async () => {
    ...
    try {
      ...
      setError(`Failed to deploy contract: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  useEffect(() => {
    ...
    const fetchUser = async() => {
        if (! session) return;
        const res = await fetch(`/api/publicProfile?username=${session.data?.user?.username}`,{
          method:"GET"
        }
        )
        const responseData=await res.json();
  ...
  const addProject = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!userData.maintainerWallet || !abi || (abi as readonly any[]).length === 0) {
            ...
            <div className="mx-auto text-center w-max-sm pt-40">
              ...
              <div className="text-sm text-neutral-600 dark:text-neutral-400 text-center max-w-2xl mx-auto space-y-4">
                <p className="text-base font-medium">
                  Execute the contract below to Create an Escrow account for the Maintainer where they can store and transact through.
                </p>
                
                <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mt-4">
                  <h3 className="font-semibold text-blue-800 dark:text-blue-200 mb-2">What is a Maintainer Wallet?</h3>
                  <p className="text-blue-700 dark:text-blue-300 text-sm">
                    A smart contract-based escrow wallet that securely holds bounty funds and automatically releases payments when issues are resolved.
                  </p>
                </div>
                ...
                <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 mt-4">
                  ...
                  <ul className="text-amber-700 dark:text-amber-300 text-xs space-y-1 text-left">
                    ...
                    <li>• Wallet address will be permanently linked to your account</li>
                    <li>• Contract includes initial balance of 100 wei for testing</li>
                  </ul>
                </div>
              </div>
                  ...
                  {contractAddress && (
                    <div className="mt-4 p-3 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg">
                      <p className="text-green-700 dark:text-green-300 text-sm">
                        ✅ Wallet created successfully! Address: <code className="bg-green-100 dark:bg-green-900 px-1 rounded">{contractAddress}</code>
                      </p>
                    </div>
                  )}
              </div>
