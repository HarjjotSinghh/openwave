...
export default function MaintainerList({ repoOwner,repoName,maintainers }: MaintainerListProps) {
  ...
  const [maintainer, setMaintainers] = useState([]);
  ...
  useEffect( () => {
    ...
    const fetchMaintainers=async() => {
        ...
        const filtered = response.data.filter(
          (collab) => !collab.permissions?.admin && (collab.permissions?.maintain || collab.permissions?.triage || collab.permissions?.push)
        );
        setAdmins(admin);
  ...
  },[session,maintainer,users])
  
    const enhancedMaintainers = maintainer.map(m => {
    const userData = users.find(user => user.id === m.login);
    return {
      ...m,
  ...
  // Check if this maintainer is in the projectMaintainers array
  return projectMaintainers && projectMaintainers.includes(maintainer.login);
});
...
const nonVerifiedfilteredProjectMaintainers = enhancedMaintainers.filter(maintainer => {
  ...
  const addContributor=async(maintain:Maintainer)=> {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-4 p-3 border rounded-md ${
                        !maintainer.isSignedUp ? "bg-muted text-muted-foreground opacity-70" : "bg-card"
                      }`}
                    >
