...
export default function Project() {
  ...
  useEffect(() => {
    const getContributionData = async () => {
      const repoResponse = await fetch(
        `/api/contributor-applications?projectName=${Repo}&username=${session?.user?.username}`,
        {
          method: "GET",
        }
      );
      const data = await repoResponse.json();
      setContributorsData(data.applications || []);
    };
    getContributionData();
  }, [session]);
  ...
  useEffect(() => {
    const fetchRepoData = async () => {
      ...
      try {
        ...
        // Always fetch repository data
        const repoResponse = await fetch(
          `/api/specific-repo?project_repository=${Repo}&username=${session?.user?.username}`,
          {
            method: "GET",
        ...
        // Only fetch issues from database if repository is private
  ...
  useEffect(() => {
    ...
    const fetchRepositoryDetails = async () => {
      try {
        ...
        // Determine the actual repository owner
        const repoOwner =
        ...
        // If repository is public, fetch data using Octokit
        if (repoData?.type === "public") {
          const [
            ...
            // Fetch collaborators (users with push access)
            octokit.request(
          ...
          // Transform GitHub issues to match the expected format
          ...
          // Update issues state with GitHub data
          ...
          // For private repositories, use the existing logic
          const [
            contributorsResponse,
            languagesResponse,
            commitsResponse,
  ...
  return (
    ...
    >
                              ...
                              return (
                                <>
                                  <div key={collab.id} className="flex">
                                    <img
                                      src={
                                        collab.avatar_url || "/placeholder.svg"
                                      }
                                      alt={collab.login}
                                      className="w-7 h-7 rounded-full"
                                    />
                            ...
                            ([lang, percentage]: [string, number]) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div
                                    className={`overflow-hidden text-sm rounded-full font-medium dark:text-white text-custom-neutral`}
                                  >
                                    <span
                ...
                <div>
                  ...
                  contributorsData[0]?.status == `approved` ? (
                    ...
                    <>
                      ...
                      {/* Issues Skeleton */}
                      <div className="border-neutral-300 dark:border-custom-dark-neutral border-2 rounded-xl p-4 mt-7 animate-pulse">
                        <div className="flex gap-2 my-auto">
                            ...
                            <div className="flex flex-col lg:flex-row lg:justify-between gap-4">
                              <div className="flex-1">
                                <div className="flex gap-2">
                                  ...
                                  <div className="w-48 h-6 bg-neutral-200 dark:bg-neutral-700 rounded"></div>
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-2">
                                  <div className="w-6 h-6 bg-neutral-200 dark:bg-neutral-700 rounded"></div>
                                  <div className="w-12 h-6 bg-neutral-200 dark:bg-neutral-700 rounded"></div>
                                </div>
                                <div className="w-32 h-10 bg-neutral-200 dark:bg-neutral-700 rounded"></div>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-6 w-full border border-neutral-300 dark:border-custom-dark-neutral rounded-lg p-4 animate-pulse">
                        <div className="w-40 h-8 bg-neutral-200 dark:bg-neutral-700 rounded mb-4"></div>
                        ...
                        <div className="space-y-2">
                          {[...Array(5)].map((_, index) => (
                            ...
                            >
                              <div className="flex items-center flex-1 min-w-0">
                                <div className="w-12 h-6 bg-neutral-200 dark:bg-neutral-700 rounded-md mr-3 shrink-0"></div>
                                <div className="w-64 h-4 bg-neutral-200 dark:bg-neutral-700 rounded flex-1"></div>
                              </div>
                              <div className="w-20 h-4 bg-neutral-200 dark:bg-neutral-700 rounded shrink-0"></div>
                            </div>
                          ))}
                        </div>
                      </div>
      ...
      {/* Modal overlay - only shows when modalOpen is true */}
      <ContributorApplicationForm repo={Repo} />
    </ThemeModalContext.Provider>
  );
}
