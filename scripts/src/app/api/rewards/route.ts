  ...
  try {
    ...
    if (
      ...
      return errorResponse(
        "Missing required fields for reward entry (projectName, Contributor_id, issue, rewardAmount)"
      );
    }
    ...
    if (isNaN(numericAmount) || numericAmount < 0) {
      return errorResponse(
        "Invalid input: rewardAmount must be a positive number."
      );
    }
...
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const filters = [];
    if (contributor)
      filters.push(like(Rewards.Contributor, `%${contributor}%`));
    ...
    const allRewards: Reward[] = await db
      .select()
      .from(Rewards)
      .where(filters.length ? and(...filters) : undefined)
      .execute();
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error occurred";
    return errorResponse(
      `Failed to fetch reward entries: ${errorMessage}`,
      500
    );
  }
}
...
export async function DELETE(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!projectName || !contributor || !issue) {
      return errorResponse(
        "Missing required parameters (projectName, contributor, issue)"
      );
    }
...
export async function PATCH(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!projectName || !Contributor_id || !issue) {
      return errorResponse(
        "Missing required fields for identification (projectName, Contributor_id, issue)"
      );
    }
    ...
    if (project_repository !== undefined)
      updateData.project_repository = project_repository;
...
export async function PUT(request: Request): Promise<NextResponse> {
  try {
    ...
    const { contributor_id, issue } = data;

    if (!contributor_id) {
      return errorResponse("Missing required field: contributor_id");
    }
    ...
    const result = await db
      ...
      .set({ withdrawn: true })
      .where(
        and(eq(Rewards.Contributor, contributor_id), eq(Rewards.issue, issue))
      )
      .execute();

    if (!result || result.length === 0) {
      return errorResponse("No matching reward entry found", 404);
    }

    return NextResponse.json({
      success: true,
      message: "Reward withdrawn status updated successfully",
    });
  } catch (error) {
    console.error("Error in updating reward entry:", error);
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error occurred";
    return errorResponse(`Failed to update reward entry: ${errorMessage}`, 500);
  }
